...
complete_df.drop_duplicates(subset=['parsed_title', 
                                    'parsed_body_ques','parsed_body_ans'],inplace=True)
complete_df.to_csv('data/complete_df.csv')
train, test = \
              np.split(complete_df.sample(frac=1, random_state=42), 
                       [int(.8*len(complete_df))])
